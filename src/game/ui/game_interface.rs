...
use crate::{
    ...
    globals::{INTERFACE_GLOBAL_PLAYER_NAME_FONT_SIZE, CHAR_SIZE, STANDARD_TILE_HEIGHT, STANDARD_TILE_WIDTH}, render::components::GameCursorRender
};
...
pub fn draw_enemy_health(
    ...
    for (health, transform) in enemies_q.iter() {
        ...
        let left =screen_position.x - ((STANDARD_TILE_WIDTH / 2) as f32);
        //let right =screen_size.x - screen_position.x;
        let top =screen_position.y - ((STANDARD_TILE_HEIGHT / 2) as f32); // REMEMBER : world = y goes from bottom to top (++)
